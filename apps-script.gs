...
var HEADERS = [
  ...
  '出生年月日','性別','護照效期到期日','手機號碼','Email','緊急聯絡人','緊急聯絡電話',
  '常客卡號','座位偏好','餐食需求','特殊協助','托運特殊物品','出發機場','簽證狀態','備註','護照照片連結'
];
...
function doPost(e) {
  ...
  try {
    ...
    sheet.appendRow([
      ...
      p.phone || '', p.email || '', p.emgName || '', p.emgPhone || '',
      p.mileage || '', p.seat || '', p.meal || '', p.assist || '', p.baggage || '',
      p.airport || '', p.visa || '', p.note || '', photoUrl
    ]);
